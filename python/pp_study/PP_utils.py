...
def _run_pp_from_uv(source_mesh, uv_init, *, max_iter_num, bound_distortion_K,
    ...
    """Standalone policy and metadata for file-based and comparison-video callers."""
    uv_final, _, history = _run_pp_controller(
        source_mesh, uv_init, execution_mode='standalone', iteration_limit=max_iter_num,
        bound_distortion_K=bound_distortion_K, convergence_rate=convergence_rate,
        return_history=return_history, iteration_callback=iteration_callback, fixed_vars=fixed_vars)
...
def run_pp_true_area_from_uv(source_mesh, uv_init, *, execution_mode,
    ...
    if not math.isfinite(convergence_rate) or convergence_rate <= 0.0:
        raise ValueError('convergence_rate must be finite and positive.')
    _configure_import_paths()
    return _run_pp_controller(
        source_mesh, uv_init, execution_mode=execution_mode, iteration_limit=iteration_limit,
        bound_distortion_K=bound_distortion_K, convergence_rate=convergence_rate,
        return_history=return_history, post_step_cb=post_step_cb)


def _run_pp_controller(source_mesh, uv_init, *, execution_mode, iteration_limit,
                       bound_distortion_K, convergence_rate, return_history,
                       iteration_callback=None, post_step_cb=None, fixed_vars=(0, 1)):
    """Shared PP loop; entry points select budget, stopping, and observer policies."""
    import numpy as np
    import mesh_energy
    import continuation_parametrization
    import extra_utils
