...
if args.state:
    ...
    v.setVars(saved_state['variables'])
...
if args.output:
    ...
    np.savez_compressed(args.output / (stem + '.npz'), coefficients=fcoeffs, variables=v.getVars(), direction=d)
    def revision(path):
    ...
    report = dict(state=str(args.state) if args.state else None, model=args.model, projection=args.projection, degree=args.degree,
                  repeats=args.repeats, threads=args.threads, wall_seconds=elapsed,
