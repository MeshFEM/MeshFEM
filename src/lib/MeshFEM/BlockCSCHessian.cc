...
BlockCSCHessian<VarStructure>::~BlockCSCHessian() = default;
...
// need to copy them...)
template<class VarStructure>
std::unique_ptr<BlockCSCHessian<VarStructure>> BlockCSCHessian<VarStructure>::construct(const VarStructure &vars) {
    std::unique_ptr<BlockCSCHessian<VarStructure>> result(new BlockCSCHessian(vars)); // can't use `make_unique` due to private constructor
    result->symmetry_mode = CSCMat::SymmetryMode::UPPER_TRIANGLE;
    return result;
}

template<class VarStructure>
std::unique_ptr<BlockCSCHessianBase> BlockCSCHessian<VarStructure>::clone() const {
    return std::make_unique<BlockCSCHessian>(*this);
}

template<class VarStructure> const BlockCSCHessian<VarStructure> &BlockCSCHessian<VarStructure>::cast(const BlockCSCHessianBase &H_base) { try { return dynamic_cast<const BlockCSCHessian<VarStructure> &>(H_base); } catch (const std::bad_cast &) { std::cerr << "dynamic_cast failed for source " << typeid(H_base).name() << ", target:" << typeid(BlockCSCHessian).name() << std::endl; throw; } }
template<class VarStructure>       BlockCSCHessian<VarStructure> &BlockCSCHessian<VarStructure>::cast(      BlockCSCHessianBase &H_base) { try { return dynamic_cast<      BlockCSCHessian<VarStructure> &>(H_base); } catch (const std::bad_cast &) { std::cerr << "dynamic_cast failed for source " << typeid(H_base).name() << ", target:" << typeid(BlockCSCHessian).name() << std::endl; throw; } }
...
template struct MESHFEM_EXPORT BlockCSCHessian<OptimizationVarStructure<1>>;
template struct MESHFEM_EXPORT BlockCSCHessian<OptimizationVarStructure<2>>;
template struct MESHFEM_EXPORT BlockCSCHessian<OptimizationVarStructure<3>>;
template struct MESHFEM_EXPORT BlockCSCHessian<OptimizationVarStructure<3, 1, 1>>;
