...
struct CholmodSparseWrapper {
    CholmodSparseWrapper(size_t cols, cholmod_sparse *mat, cholmod_common &c)
        : n(cols), m_mat(mat), m_c(c) { if (mat == nullptr) throw std::runtime_error("Null matrix passed"); }
...
public:
    ...
    void solveRaw(const Real *b, Real *x, int sys = CHOLMOD_A) {
        ...
        cholmod_l_solve2(sys, m_L, &cholb, NULL, &cholx_ptr, NULL, &m_Y, &m_E, &m_c);
