...
Lagrange multipliers $\lambda\colon \overline{\Omega}(p) \to \mathbb{R}^n$ for
the volume and Neumann boundary constraints in (\ref{eqn:state}), but set $\lambda
= 0$ on $\Gamma_D$  since we do not need a term in the Lagrangian for the
Dirichlet condition (we will always
...
Unfortunately, all existing literature seems to rely on this boundary condition;
though \cite{Allaire2008909} mentions that ``an equilibrium condition on g'' is
needed in the absence of the Dirichlet boundary, they do not discuss how to keep
the adjoint problem well-posed.
...
\begin{itemize}
    ...
    \item Predicting how worst-case volume forces or surface tractions change
        with boundary movement.
\end{itemize}
...
\begin{appendices}
    ...
    derivations are straightforward in index notation. First, we derive the one
    for
    $
    \vint{\lambda \cdot \left(\div \left[ C:\epsilon(u)\right]\right)}
    $, i.e. $ \vint{\lambda_i \pder{}{x_j}\big[C_{ijkl} \epsilon_{kl}\big]}. $
    From product rule:
    $$
    \vint{\lambda_i \pder{}{x_j}C_{ijkl} \epsilon_{kl}(u)} =
    \vint{\pder{}{x_j} \big[\lambda_i C_{ijkl} \epsilon_{kl}(u)\big] -
    \pder{\lambda_i}{x_j} C_{ijkl} \epsilon_{kl}(u)}.
    $$
    Notice that $\pder{\lambda_i}{x_j} C_{ijkl} = \epsilon_{ij}(\lambda) C_{ijkl}$ by
    the symmetry $C_{ijkl} = C_{jikl}$. This plus Gauss' theorem for the first
    term gives:
    $$
    \sint{\lambda_i C_{ijkl} \epsilon_{kl}(u)\hat{n}_j} -
    \vint{\epsilon_{ij}(\lambda)C_{ijkl} \epsilon_{kl}(u)}.
    $$
    ...
    \label{eqn:IBP_2}
    \end{align}
    In component notation we have, by symmetry $C_{ijkl} = C_{ijlk}$,
    $$
    \vint{\gamma_{ij} C_{ijkl} \epsilon_{kl}(\phi)} = \vint{\gamma_{ij} C_{ijkl} \pder{\phi_k}{x_l}}.
    $$
    By product rule, the expression becomes:
    $$
    \vint{\pder{}{x_l}\big[\gamma_{ij} C_{ijkl} \phi_k \big] - \pder{\gamma_{ij} C_{ijkl}}{x_l} \phi_k}.
    $$
    Applying Gauss' theorem to the left term and using major symmetry $C_{ijkl} = C_{klij}$:
    $$
    \sint{\gamma_{kl} C_{ijkl} \phi_i \hat{n}_j} - \vint{\pder{\gamma_{kl}C_{ijkl} }{x_j} \phi_i}.
    $$
...
+ \vint{\pder{}{p_i} \div \big[\tilde{h}(p, x)\tilde{n} \big]}
    ...
    $$
    \sint{\left[\pder{\tilde{h}(p, x)}{\hat{n}} + h(p, x) H \right]\big(v_i(p) \cdot \hat{n}\big)}
    ...
    If we can define $\tilde{h}$ so that $\evalat{\pder{\tilde{h}(p, x)}{\hat{n}}}{\boundary} = 0$
