...
struct MeshBindings<FEMMesh<3, _Deg, _EmbeddingSpace>> : public MeshBindingsBase<FEMMesh<3, _Deg, _EmbeddingSpace>> {
    ...
    static MeshBindingsType<Mesh> bind(py::module &module, py::module &detail_module) {
        ...
        mesh_bindings
            ...
            .def("shrunkenTetVisualizationField", [](const Mesh &m, const Eigen::MatrixXd &f) { throw std::runtime_error("Unimplemented"); }, "Convert a per-vertex or per-element field into a per-visualization-geometry field (called internally by MeshFEM visualization)", py::arg("perEntityField"))
        ;
        return mesh_bindings;
    }
};
