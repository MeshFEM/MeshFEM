...
void bind(py::module &m, py::module &detail_module) {
    ...
    m.def("IPCObjectiveTerm", [](std::shared_ptr<NewtonVarsBase> vars, CollisionMesh cm, const ObstaclesCollection &obstacles) { return std::make_shared<IPCO>(vars, cm, obstacles); }, py::arg("vars"), py::arg("collisionMesh"), py::arg("obstacles") = ObstaclesCollection(), py::call_guard<py::scoped_ostream_redirect, py::scoped_estream_redirect>());

    // Convenience method for constructing from an ElasticObject
    m.def("IPCObjectiveTerm", [](std::shared_ptr<EO> eo, const ObstaclesCollection &obstacles) { return std::make_shared<IPCO>(eo, eo->getCollisionMesh(), obstacles); }, py::arg("eo"), py::arg("obstacles") = ObstaclesCollection(), py::call_guard<py::scoped_ostream_redirect, py::scoped_estream_redirect>());
