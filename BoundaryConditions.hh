...
public:
    ...
    void writeConditions(const std::string &cpath) const {
        std::ofstream outFile(cpath);
        if (!outFile.is_open())
            std::cout << "Couldn't open BC file:" << cpath << '\'' << std::endl;
        else
            writeConditions(outFile);
    }
            
    void writeConditions(std::ostream &os) const {
        os << "{ \"regions\": [" << std::endl;
    ...
    void readConditions(const std::string &cpath) {
    ...
    void readConditions(std::istream &is) {
        ptree pt;
        read_json(is, pt);
        ...
        BOOST_FOREACH(const ptree::value_type &val, regions) {
            ...
            std::string type = tcond.get_child("type").get_value<std::string>();
            Condition c;
            ...
            else    throw std::runtime_error(std::string("Invalid type '") + type + "'");
