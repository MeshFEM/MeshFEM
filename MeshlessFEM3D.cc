...
class MeshlessFEM3D<_Model>::PerElementOrthotropicStiffnessIntegrand
{
public:
...
void MeshlessFEM3D<_Model>::m_assembleStiffnessMatrix(TMatrix &K,
    ...
    K.m = K.n = 3 * numDOFs;
    PerElementOrthotropicStiffnessIntegrand stiff(m_E, m_model);
    ...
    for (size_t e = 0; e < m_elementGrid.numElements(); ++e) {
        ...
        stiff.configure(b.dimensions(), exact);
        if (!exact) m_quadrature.integrate(stiff, b);
...
void MeshlessFEM3D<_Model>::m_computePerElementDisplacementStrainMap() {
    ...
    for (size_t e = 0; e < numElements; ++e) {
        _BBox b = m_elementGrid.elementBoundingBox(e);
        bool exact = m_exactFullElements && m_elementGrid.elementIsFull(e);
        gradPhi.configure(b.dimensions(), exact);
