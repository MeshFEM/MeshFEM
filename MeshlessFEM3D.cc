...
public:
    ...
    PerElementOrthotropicStiffnessDensity(const ETensor &E, const Model &model)
        : m_model(model), m_E(E) { clear(); setDimensions(Vector::Zero()); }
...
periodicHomogenize(Timer *timer, _MSHWriter *mshWriter) {
    ...
    for (size_t i = 0; i < 6; ++i) {
        ...
        for (size_t e = 0; e < m_elementGrid.numElements(); ++e) {
            ...
            m_elementData[e].displacementToStress(w_ij[i], cornerIndices, m_E,
                                                  stress);
