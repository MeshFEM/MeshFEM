...
void writeBoundaryConditions(const string &cpath,
                             const vector<ConstCondPtr<_Vec> > &conds) const {
...
void writeBoundaryConditions(ostream &os,
    ...
    for (size_t i = 0; i < numConditions(); ++i) {
        ...
        os << "\", \"value\": ["
           << c.value[0] << ", " << c.value[1] << ", " << ((N == 2) ?  0 : c.value[2])
           << "], \"box\": { \"minCorner\": ["
...
vector<CondPtr<_Vec> > readBoundaryConditions(const string &cpath) {
    ifstream inFile(cpath);
    if (!inFile.is_open()) throw runtime_error("Couldn't open BC file");
    return readConditions(inFile);
}

template<typename _Vec>
vector<CondPtr<_Vec> > readBoundaryConditions(istream &is) {
    ptree pt;
    read_json(is, pt);
    ...
    BOOST_FOREACH(const ptree::value_type &val, regions) {
        ...
        BBox<_Vec> region;
        region.minCorner = truncateFrom3D<_Vec>(parseVectorLenient<Vector>(tcond.get_child("box.minCorner")));
        region.maxCorner = truncateFrom3D<_Vec>(parseVectorLenient<Vector>(tcond.get_child("box.maxCorner")));
        Vector3D value = parseVectorLenient(tcond.get_child("value"));

        BoundaryCondition *c;
        if      (type == "pressure")  c = new   NeumannCondition(region, value[0]);
        else if (type == "traction")  c = new   NeumannCondition(region, truncateFrom3D<_Vec>(value));
        else if (type == "dirichlet") c = new DirichletCondition(region, truncateFrom3D<_Vec>(value));
        else if (type == "target")    c = new    TargetCondition(region, truncateFrom3D<_Vec>(value));
...
template<> void writeBoundaryConditions<Vector3D>(ostream &os,
                             const vector<ConstCondPtr<Vector3D> > &conds);
template<> vector<CondPtr<Vector3D> > readBoundaryConditions<Vector3D>(const string &cpath);
template<> vector<CondPtr<Vector3D> > readBoundaryConditions<Vector3D>(istream &is); 
...
template<> void writeBoundaryConditions<Vector2D>(ostream &os,
                             const vector<ConstCondPtr<Vector2D> > &conds);
template<> vector<CondPtr<Vector2D> > readBoundaryConditions<Vector2D>(const string &cpath);
template<> vector<CondPtr<Vector2D> > readBoundaryConditions<Vector2D>(istream &is); 
