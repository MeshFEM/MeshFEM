    ...
    for ij in {0..2}; do
        for sample in {0..1}; do
            for comp in {x,y,norm}; do
                gnuplot -e "data_source='< ./errorData.py $i \"w${ij}_$comp[$sample]\"'; name='fluctuation displacement $ij at sample point $sample ($comp)'; title_suffix=' (Mesh corner angle $cornerAngle)'; png_path='$i/w${ij}_$comp.$sample.png'" plot_relerrors.gpi
            done
        done
    done
    for ij in {0..2}; do
