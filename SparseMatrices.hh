...
public:
    ...
    ConstrainedSystem(TripletMatrix<Triplet<_Real> > &C,
                   const std::vector<_Real> &constraintRHS)
        : m_cached(false), m_C(NULL), m_CFactors(NULL) {
        setSystem(C, constraintRHS);
    }
    ...
    void solve(const _Vec &f, std::vector<_Real> &u) {
        ...
        if (!m_cached) {
            ...
            BENCHMARK_ADD_MESSAGE("Peak factorization memory (MB):\t" + std::to_string(m_CFactors->peakMemoryMB()));
            m_cached = true;
        }
