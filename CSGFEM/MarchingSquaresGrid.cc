    ...
    while (true) {
        ...
        switch(dir) {
            case MS_LEFT:
                ...
                if (!(s <= 1.0 && s >= 0.0)) {
                    Vector2D va(vertexPosition(corners[0])),
            ...
            case MS_RIGHT:
                ...
                s = a / (a - b); // Approximate zero crossing
                if (!(s <= 1.0 && s >= 0.0)) {
                    Vector2D va(vertexPosition(corners[1])),
            ...
            case MS_UP:
                ...
                s = a / (a - b); // Approximate zero crossing
                if (!(s <= 1.0 && s >= 0.0)) {
                    Vector2D va(vertexPosition(corners[3])),
                           vb(vertexPosition(corners[2]));
...
//  other. This is a conservative merge--we are really merging sequences of
                                         ...
                                         Polygon_t &boundary) const
{
    ...
    const std::vector<Vector2D> &pts = boundary.points;
    std::vector<Vector2D> mergedBoundary;
    ...
    for (size_t offset = 0; offset < pts.size(); ++offset) {
        ...
        do {
            ...
            if (!processed[nextI]) {
                Vector2D avg = (p + pts[nextI]) / (seqLen + 1);
                bool mergeable = true;
                for (size_t j = 0; j < seqLen + 1; ++j) {
                    Vector2D dist = pts[(i + j) % pts.size()] - avg;
